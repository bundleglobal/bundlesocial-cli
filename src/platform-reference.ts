 ...
 * bundle.social API accepts. Exposed via the `platforms:describe` command so an
 * agent (or a human) can look up exactly which fields a platform needs when
 * building `--data` / `--platform-settings` instead of guessing.
 *
 * SYNC: the field tables here mirror PROVIDER_SETTINGS.md and
...
export interface PlatformReferenceEntry {
  ...
  /** The `data.<PLATFORM>` schema used by posts:create / posts:schedule / posts:update. */
  post: OperationReference;
  /** The `data.<PLATFORM>` schema used by comments:create / comments:update (omitted when unsupported). */
...
const POST: Record<Platform, OperationReference> = {
  TWITTER: {
    ...
    summary: "Post a tweet. For a thread, post the first tweet then add replies via comments:create.",
    fields: [
  ...
  INSTAGRAM: {
    ...
    fields: [
      ...
      { name: "tagged", type: "{ username: string, x?: number, y?: number }[]", required: false, notes: "People tags for a single-image post." },
      {
        ...
        type: "string",
        required: false,
        notes: "A location id — discover with `integrations:trigger instagram:locations`.",
      },
      ...
      },
      {
        ...
        required: false,
        notes: "Reels only — get an audio_id from `integrations:trigger instagram:audio`. Volumes 0–100.",
      },
  ...
  TIKTOK: {
    ...
    fields: [
      ...
      { name: "uploadToDraft", type: "boolean", required: false, notes: "Upload as a TikTok draft instead of publishing." },
      {
        ...
        required: false,
        notes: "Commercial music — get a song_clip_id from `integrations:trigger tiktok:trending-music`. Volumes 0–100; start/end in ms.",
      },
    ],
  ...
  YOUTUBE: {
    ...
    fields: [
      ...
      { name: "text", type: "string", required: false, notes: "The video TITLE (the post `--title` flag also maps here)." },
  ...
  REDDIT: {
    ...
    fields: [
      ...
      { name: "nsfw", type: "boolean", required: false, notes: "Mark NSFW." },
      {
        ...
        type: "string",
        required: false,
        notes: "Required if the subreddit requires a flair — get options with `integrations:trigger reddit:flairs`.",
      },
    ],
  ...
  DISCORD: {
    ...
    fields: [
      { name: "channelId", type: "string", required: true, notes: "The connected server channel (an integration \"channel\" id — see integrations:list)." },
  ...
  SLACK: {
    ...
    fields: [
      { name: "channelId", type: "string", required: true, notes: "The connected workspace channel (an integration \"channel\" id — see integrations:list)." },
...
const NOTES: Partial<Record<Platform, string[]>> = {
  ...
  LINKEDIN: ["Discover mention URNs with `integrations:trigger linkedin:mentions` and put them in `text`."],
  REDDIT: [
    "Before posting, run `integrations:trigger reddit:requirements` to learn title length limits, allowed post types and whether a flair is required.",
  ],
  YOUTUBE: ["There is no `categoryId` in the post `data`; fetch categories with `integrations:trigger youtube:categories`."],
  INSTAGRAM: [
  ...
  GOOGLE_BUSINESS: ["Set the location up in the dashboard first; details/categories/hours are managed via `integrations:trigger` helpers."],
  SNAPCHAT: [
...
export const MEDIA_NOTES: string[] = [
  ...
  "Upload media first (`media:upload <url|path>` → id), or let `posts:create -m` do it; then put ids in `uploadIds` (and carouselItems[].uploadId / mediaItems[].uploadId).",
  "Large videos can take a while to process server-side after upload — a newly created post may sit in PROCESSING before going POSTED. Use `posts:get` to check status; `posts:retry` for a transient platform failure.",
];
