 ...
 * mentions, GBP locations, etc. The platform is implied by the method name; all
 * are team-scoped, with extra parameters supplied via `--data` (JSON).
 */
export interface IntegrationTool {
  /** Stable method id, e.g. `reddit:flairs`. */
  id: string;
  platform: string;
  description: string;
  /** Required keys the caller must supply in `--data` (besides teamId). */
  requiredParams?: string[];
  /** Optional keys understood for `--data`. */
...
export const INTEGRATION_TOOLS: IntegrationTool[] = [
  ...
  },
  {
    ...
    run: (client, teamId, p) =>
      client.misc.miscGoogleBusinessGetLocation({
        teamId,
        ...(Array.isArray(p.fields) ? { fields: p.fields as never } : {}),
      }),
  },
  {
    id: "gbp:categories",
...
export function findIntegrationTool(method: string): IntegrationTool {
...
export async function runIntegrationTool(
  ...
  const tool = findIntegrationTool(method);
  const missing = (tool.requiredParams ?? []).filter((key) => params[key] === undefined || params[key] === null || params[key] === "");
  if (missing.length > 0) {
    throw new CliError(
      "MISSING_PARAMS",
      `Integration tool "${tool.id}" requires: ${missing.join(", ")}. Pass them as JSON, e.g. --data '{"subreddit":"r/test"}'.`,
